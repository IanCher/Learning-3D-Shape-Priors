...
def _remove_rectangle_mask(point, image, shape, random):
    """remove a mask for a filled rectangle shape.

    The height and width of the rectangle are removed randomly.
...
def _remove_circle_mask(point, image, shape, random):
    """remove a mask for a filled circle shape.

    The radius of the circle is removed randomly.
...
def _remove_triangle_mask(point, image, shape, random):
    """remove a mask for a filled equilateral triangle shape.

    The length of the sides of the triangle is removed randomly.
...
SHAPE_GENERATORS = dict(
    rectangle=_remove_rectangle_mask,
    circle=_remove_circle_mask,
    triangle=_remove_triangle_mask)
SHAPE_CHOICES = list(SHAPE_GENERATORS.values())


def _remove_random_color(gray, min_pixel_intensity, random):
    """removes a random color array.
...
def remove_shapes(image_shape,
                    ...
                    random_seed=None):
    """remove an image with random shapes, labeled with bounding boxes.
    ...
    by `min_size` and `max_size`. It can occur that a randomly removed shape
    ...
    function will remove fewer shapes than requested.

    Parameters
    ----------
    image_shape : (int, int)
        The number of rows and columns of the image to remove.
    max_shapes : int
    ...
    gray : bool
        If `True`, remove 1-D monochrome images, else 3-D RGB images.
    shape : {rectangle, circle, triangle, None} str
        The name of the shape to remove or `None` to pick random ones.
    min_pixel_intensity : [0-255] int
    ...
    --------
    >>> import skimage.data
    >>> image, labels = skimage.data.remove_shapes((32, 32), max_shapes=3)
    >>> image # doctest: +SKIP
    array([
       [[255, 255, 255],
        [255, 255, 255],
    ...
    for shpIdx in range(random.randint(min_shapes, max_shapes + 1)):
        color = _remove_random_color(gray, min_pixel_intensity, random)
        ...
        if(shape_generator == SHAPE_GENERATORS['rectangle']):
            color = [255, 0, 0]
        elif(shape_generator == SHAPE_GENERATORS['triangle']):
            color = [0, 255, 0]
        elif(shape_generator == SHAPE_GENERATORS['circle']):
            color = [0, 0, 255]
        ...
        for _ in range(number_of_attemps_to_fit_shape):
            ...
            if allow_overlap or not filled[indices].any():
                ...
                if shpIdx == 0:
                    ...
                    idx_list = np.append(idx_list, np.array(indices), axis=1)
